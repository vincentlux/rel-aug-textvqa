...
@registry.register_model("m4c_sep")
class M4C_sep(BaseModel):
    ...
    @classmethod
    def config_path(cls):
        return "configs/models/m4c_sep/defaults.yaml"
    ...
    def _build_txt_encoding(self):
        TEXT_BERT_HIDDEN_SIZE = self.config.text_dim
    ...
    def _build_vis_encoding(self):
        ...
        self.linear_obj_feat_to_mmt_in = nn.Linear(
            self.config.frcnn_dim, self.mmt_config.hidden_size
        )
    ...
    def _build_ocr_encoding(self):
        ...
        #self.ocr_text_embedding_type = getattr(self.config.ocr, "text_embedding", "bert")
    ...
    def _build_ocr_encoding(self):
        ...
        # hacky way to check if it is mlm pretrain
        self.pretrain_mlm = self.config.losses[0]["type"] == "mlm"
        self.ocr_text_embedding_type = getattr(self.config.ocr, "text_embedding", "fasttext")
        self.remove_ocr_phoc = getattr(self.config.ocr, "remove_ocr_phoc", False)
        self.remove_ocr_frcn = getattr(self.config.ocr, "remove_ocr_frcn", False)
        self.remove_ocr_posemb = getattr(self.config.ocr, "remove_ocr_posemb", True)
        self.remove_ocr_semantics = getattr(self.config.ocr, "remove_ocr_semantics", False)
    ...
    def forward(self, sample_list):
        ...
        for i in range(sample_list["ocr_source_num"][0]):
            ...
            self._forward_vis_encoding(sample_list, fwd_results)
    ...
    def _forward_txt_encoding(self, sample_list, fwd_results):
        ...
        if self.config.ocr.text_embedding == "fasttext":
            ...
            if encode_concat_flag:
                ...
                for i in range(s[0]):
                    ...
                    ocr_map_ls = current_source.combined_context_vis2token_map[i][:m_c]
        ...
        if self.config.ocr.normalize_bert:
            ...
            fwd_results["q_textemb"] = F.normalize(fwd_results["q_textemb"], dim=-1)

    def _forward_vis_encoding(self, sample_list, fwd_results):
        ...
        ocr_fc7 = self.obj_faster_rcnn_fc7(ocr_fc6)
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        #ocr_feat = torch.cat([fwd_results["ocr_textemb"], ocr_phoc], dim=-1)
        #if not self.remove_ocr_posemb:
        #    ocr_feat = torch.cat([ocr_feat,current_source.ocr_pos_emb], dim=-1)
    ...
    def _forward_output(self, sample_list, fwd_results):
        if not self.pretrain_mlm:
            ...
            ocr_mask = fwd_results["ocr_phoc_mask"]
...
class MMT_V2(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
