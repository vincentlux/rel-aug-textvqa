...
from mmf.models.transformers.backends.oscar import BertImgModel, CaptionBertEncoder


logger = logging.getLogger(__name__)


@registry.register_model("m4c_oscar")
class M4COscar(BaseModel):
    ...
    @classmethod
    def config_path(cls):
        return "configs/models/m4c_oscar/defaults.yaml"
    ...
    def _build_txt_encoding(self):
        ...
        # TODO: add oscar config in yaml and use here
    ...
    def _build_ocr_encoding(self):
        ...
        self.linear_ocr_to_mmt_in = nn.Linear(self.config.ocr.mmt_in_dim, 2054)
    ...
    def _build_mmtoscar(self):
        ...
        if config.from_pretrained:
            self.mmtoscar = MMTOscar.from_pretrained(model_path, config=config)
        else:
            self.mmtoscar = MMTOscar(config)

        # allow specifying a different/scaled lr for multimodal transformer
        self.finetune_modules.append(
            {"module": self.mmtoscar, "lr_scale": self.config.lr_scale_mmt}
        )
            ...
            if self.training:
                ...
                if i == 0:
                    ...
                    if self.pretrain_mlm:
                        ...
                        scores = torch.where((max_conf > fwd_results["conf"]).unsqueeze(-1).unsqueeze(-1), scores,
                                             fwd_results["scores"])
    ...
    def _forward_txt_encoding(self, sample_list, fwd_results):
        ...
        if self.config.ocr.text_embedding == "fasttext":
            ...
            # Object embedding
            obj_rawtextemb = self.text_bert(txt_inds=sample_list.obj_bert_context,
                                            txt_mask=sample_list.obj_bert_input_mask)
            ...
            if encode_concat_flag:
                # TODO: call mmtoscar with forward_embeddings=True
                if self.config.from_pretrained:
                    combined_rawtextemb = self.mmtoscar(input_ids=current_source.bert_combined,
                                                    input_mask=current_source.bert_combined_mask,
                                                    forward_embeddings=True)
                else:
                    combined_rawtextemb = self.text_bert(txt_inds=current_source.bert_combined,
                                                     txt_mask=current_source.bert_combined_mask)
                ...
                raise NotImplementedError("not implemented yet for mmtoscar")
                ...
                # OCR embedding
                ocr_rawtextemb = self.text_bert(txt_inds=current_source.bert_context,
                                                txt_mask=current_source.bert_input_mask)
        ...
        if self.pretrain_mlm:
            ...
            if self.training:
                ...
                fwd_results["prev_inds"] = torch.zeros_like(
                    sample_list.train_prev_inds[:, sample_list.current_source, :])
                ...
                if self.config.beam_size == 1:
                    ...
                    for sample_id in range(batch_size):
                        ...
                        fwd_results_feed["obj_mmt_in"] = fwd_results_feed["obj_mmt_in"].repeat(self.config.beam_size, 1,
                                                                                               1)
                        fwd_results_feed["obj_mask"] = fwd_results_feed["obj_mask"].repeat(self.config.beam_size, 1)
                        fwd_results_feed["ocr_mmt_in"] = fwd_results_feed["ocr_mmt_in"].repeat(self.config.beam_size, 1,
                                                                                               1)
                        ...
                        for dec_step in range(dec_step_num):
                            ...
                            for i, seq in enumerate(seq2keep):
                                if seq[-1] == self.answer_processor.EOS_IDX or dec_step == dec_step_num - 1:
                                    ...
                                    cand_score.append(
                                        seqprob[i].item() / (seq.size(0) ** self.config.beam_length_penalty))
                                    dead += 1
                                else:
...
class MMTOscar(BertImgModel):
    ...
    def forward(
            ...
            fixed_ans_emb=None,
            prev_inds=None,
            input_ids=None, # for embeddings only
            input_mask=None,
            forward_embeddings=False,
            pretrain_mlm=None,
    ):
        ...
        if forward_embeddings:
            ...
            extended_attention_mask = extended_attention_mask.repeat(
                1, 1, input_mask.size(1), 1
            )
            # flip the mask, so that invalid attention pairs have -10000.
            extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
            embedding_output = self.encoder(embedding_output,
                                            extended_attention_mask,
                                            head_mask=head_mask)
            return embedding_output[0]
        else:
            # assemble img_feats from
            # obj_emb and ocr_emb
            # img_feats = torch.cat([obj_emb, ocr_emb], dim=1)
            # img_mask = torch.cat([obj_mask, ocr_mask], dim=1)
            ...
            if not pretrain_mlm:
                ...
                raise NotImplementedError("not implemented for mlm yet")
                encoder_inputs = torch.cat([txt_emb, obj_emb, ocr_emb, img_emb], dim=1)
                attention_mask = torch.cat([txt_mask, obj_mask, ocr_mask, img_mask], dim=1)
            ...
            # ocr indexes should use map to get
            # same output location as obj_txt_emb
            ...
            # TODO:
            #  1. try using pretrained weights, with textbert as init for ocr encoder
            #  2. try decoding using commented out version
            #  3. must do: merge obj pretrain to current code
            # (TIP: add lr steps?)
            ...
            if not pretrain_mlm:
                ...
                if self.use_txtocr_output:
                    ...
                    m_c = 50
                    context_cat_ls = []
                    for i in range(mmt_txt_output.shape[0]):
                        ocr_map_ls = ocr_map[i][:m_c]
                        ocr_token_rep = F.pad(mmt_seq_output[i][ocr_map_ls],(0,0,0,m_c-len(ocr_map_ls)),"constant",0)
                        context_cat_ls.append(ocr_token_rep)
                    mmt_ocr_output = torch.stack(context_cat_ls, dim=0)
                else:
                    mmt_ocr_output = mmt_seq_output[:, ocr_begin:ocr_end]
                results = {
